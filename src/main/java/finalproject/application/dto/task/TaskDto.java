...
public class TaskDto extends TaskListDto {
  ...
  public TaskDto(Task task) {
    ...
    this.contents = task.getAllTaskContent().stream()
            .map(ContentDto::new).toList().toArray(new ContentDto[0]);
    this.files = task.getAllTaskFiles().stream()
            .map(FileDto::new).toList().toArray(new FileDto[0]);
    this.comments = task.getComments().stream()
            .map(CommentDto::new).toList().toArray(new CommentDto[0]);
